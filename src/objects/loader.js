...
export class AppLoader extends Phaser.GameObjects.Group {
  ...
  init(_url, loaderFont) {
    ...
    this.loaderCover.fillStyle(0x000000, 1);
    this.loaderPath.fillStyle(0x000000, 1);
    this.loaderBG.fillStyle(0x000000, 1);
    ...
    for (var i = 0; i <= 50; i++) {
      this.loaderPath.fillStyle(0xffffff, 1 - (1 * i) / 100);
    ...
    this.loadPer = this.scene.make
      .text({
        x: this.c_w * 0.5,
        y: this.c_h * 0.5 * 1.4 + 150 * this.scaleFact,
        text: "0%",
        origin: {
          x: 0.5,
          y: 0.5,
  ...
  addLoaderLogo(_key) {
    ...
    this.loaderLogo.setScale(0.85 * Global.dpr);
